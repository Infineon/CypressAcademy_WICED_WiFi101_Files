/* Connect to httpbin.org using HTTPS and send a POST request to /anything */
#include <stdlib.h>
#include "wiced.h"
...
void application_start( void )
{
    ...
    /* Read certificate from file system and initialize */
    const char* root_cert;
    uint32_t cert_size;
    resource_get_readonly_buffer( &resources_apps_DIR_ww101_DIR_httpbin_DIR_ca_certificate_cer, 0, 10000, &cert_size, (const void **) &root_cert );

    result = wiced_tls_init_root_ca_certificates( root_cert, cert_size );
    ...
    WPRINT_APP_INFO(( "Root Certificate Initialized\n"));
