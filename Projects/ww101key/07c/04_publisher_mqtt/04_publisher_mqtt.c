...
void application_start( void )
{
    ...
    resource_get_readonly_buffer( &resources_apps_DIR_aws_DIR_iot_DIR_publisher_DIR_client_cer, 0, MQTT_MAX_RESOURCE_SIZE, &size_out, (const void **) &security.cert );
    if(size_out < 64)
    {
    ...
    resource_get_readonly_buffer( &resources_apps_DIR_aws_DIR_iot_DIR_publisher_DIR_privkey_cer, 0, MQTT_MAX_RESOURCE_SIZE, &size_out, (const void **) &security.key );
    if(size_out < 64)
    {
    ...
    do
    {
        ...
        while ( 1 )
        {
            ...
            if ( pub_in_progress == 1 )
            {
                ...
                do
                {
                    ret = mqtt_app_publish( mqtt_object, WICED_MQTT_QOS_DELIVER_AT_LEAST_ONCE, (char*) WICED_TOPIC, (uint8_t*) msg, strlen( msg ) );
                    retries++ ;
